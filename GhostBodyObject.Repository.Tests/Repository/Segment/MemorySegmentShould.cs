...
namespace GhostBodyObject.Repository.Tests.Repository.Segment
{
    public unsafe class MemorySegmentShould
    {
        ...
        {
            var segment = MemorySegment.NewInMemory(0);
        ...
        {
            int capacity = 2048;
            var segment = MemorySegment.NewInMemory(0, capacity);
            Assert.NotNull(segment);
        ...
        {
            Assert.Throws<InvalidOperationException>(() => MemorySegment.NewInMemory(0, 100));
        }

        [Fact]
        public void AllocateMemory()
        {
            var segment = MemorySegment.NewInMemory(0, 2048);
            int allocSize = 128;
        ...
        {
            var segment = MemorySegment.NewInMemory(0, 4096);
